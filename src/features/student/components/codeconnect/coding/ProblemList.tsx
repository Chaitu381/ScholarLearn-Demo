...
export function ProblemList({
  ...
  const codingStats = codeconnectCategoryStats.find(
    (stats) => stats.category === "coding",
  );
  ...
  const filteredProblems = useMemo(() => {
    ...
    return problems.filter((problem) => {
      ...
      const matchesTopic =
        activeTopic === "All" || problem.topic === activeTopic;

      const matchesDifficulty =
        activeDifficulty === "all" || problem.difficulty === activeDifficulty;

      const matchesStatus = matchesStatusFilter(problem, activeStatus);
  ...
  }, [activeDifficulty, activeStatus, activeTopic, problems, searchTerm]);

  const averageAccuracy = Math.round(
    problems.reduce((total, problem) => total + problem.accuracy, 0) /
  ...
  const accuracy = codingStats?.accuracy ?? averageAccuracy;
  const streakDays = codingStats?.streakDays ?? 0;

  const totalAvailablePoints = filteredProblems.reduce(
    (total, problem) => total + getProblemPoints(problem),
    0,
  );
        ...
        <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_auto] xl:items-center">
          <div className="min-w-0">
            ...
            <p className="mt-1 max-w-2xl text-[13px] font-bold leading-5 text-text-secondary">
              Pick a problem, filter by topic or difficulty, and start solving
              in the CodeConnect workspace.
            </p>
          </div>
      ...
      <section className="rounded-[1.5rem] border border-border bg-surface p-3 shadow-sm">
        <div className="grid gap-2 xl:grid-cols-[minmax(260px,1fr)_auto_auto] xl:items-center">
          <label className="flex h-11 items-center gap-3 rounded-2xl border border-border bg-surface-soft px-4 transition focus-within:border-primary focus-within:bg-surface">
            ...
            <span className="sr-only">Search coding problems</span>
        ...
        {filteredProblems.length > 0 ? (
          <div className="grid gap-2.5">
            {filteredProblems.map((problem) => {
              const active = activeProblemId === problem.id;
              const statusTone = getStatusTone(problem.status);
              ...
              return (
                <article
                  ...
                  onKeyDown={(event) =>
                    handleProblemKeyDown(event, problem.id)
                  }
                  role="button"
                  tabIndex={0}
                >
                  <div className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_360px_170px] xl:items-center">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      <InlineMetric label="Topic" value={problem.topic} />
                      ...
                      <InlineMetric
                        label="Points"
                        value={`${getProblemPoints(problem)} XP`}
                      />
                    </div>
                    ...
                    <div className="flex items-center justify-start gap-2 xl:justify-end">
                      ...
                      >
                        {getAttemptBadgeLabel(problem.status)}
                      </span>
...
function matchesStatusFilter(
  ...
  if (activeStatus === "bookmarked") {
    return Boolean(problem.bookmarked);
  }

  if (activeStatus === "attempted") {
    return problem.status === "in-progress" || problem.status === "review";
  }

  return problem.status === activeStatus;
}

function getProblemPoints(problem: CodingProblem) {
...
function getStatusLabel(status: PracticeStatus) {
  if (status === "solved") {
    return "Solved";
  }
...
function getAttemptBadgeLabel(status: PracticeStatus) {
  if (status === "solved") {
    return "Solved";
  }
